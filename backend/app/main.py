...
def prototype_ui():
    ...
    <body>
        ...
        <header>
          <div class="header-inner">
            ...
            <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
              ...
              <a href="/demos" style="text-decoration:none;font-family:'JetBrains Mono',monospace;font-size:11px;letter-spacing:1.5px;color:var(--warn);padding:7px 14px;background:rgba(255,176,32,0.10);border:1px solid rgba(255,176,32,0.50);border-radius:99px;font-weight:900;">🎬 9 기능 데모</a>
        ...
        <div class="av-wrap">
          ...
          <!-- 자가검증 6 컴포넌트 (단, 메인 대시보드는 실시간 수집/통계 핵심만) -->
          <section class="av-panel" style="margin-bottom:14px;">
            <h3>🩺 파이프라인 건강 — 자가검증 6 컴포넌트<span class="av-badge"><span class="ring"></span>/fleet/verify</span></h3>
            <div class="av-desc">JSON manifest / 이미지 무결성 / cv2 PII 마스킹 / fusion schema / 최근 활동 / 위치인식 정확성.</div>
            <div id="avHl" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:7px;">
              <div style="grid-column:1/-1;text-align:center;color:#7C8AA8;font-size:11px;padding:14px;">⏳ /fleet/verify 로딩…</div>
            </div>
          </section>

          <!-- 기능 분류 안내 (메인 대시보드 = 실시간/통계만, 세부 기능은 별도 페이지) -->
          <div style="padding:14px 18px;background:rgba(124,58,237,0.05);border:1px solid rgba(124,58,237,0.20);border-radius:12px;">
            <div style="font-family:'JetBrains Mono',monospace;font-size:10px;letter-spacing:1.8px;color:#7C3AED;font-weight:900;margin-bottom:8px;">// 세부 기능 페이지 (메인 대시보드는 실시간 통계만)</div>
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:8px;">
              <a href="/demos" style="text-decoration:none;background:#070C16;border:1px solid #1A2438;border-radius:8px;padding:10px 12px;display:block;transition:all .15s;">
                <div style="font-size:11px;font-weight:900;color:#FFB020;letter-spacing:0.4px;">🎬 9 기능 데모</div>
                <div style="font-size:9.5px;color:#7C8AA8;margin-top:3px;">AuraView 데모 · BEV 3D · Fleet Learning · Capability Matrix · 사고 재현 · K-MaaS · V2V · ⑩ 공공데이터 라이브</div>
              </a>
              <a href="/fusion/sources" style="text-decoration:none;background:#070C16;border:1px solid #1A2438;border-radius:8px;padding:10px 12px;display:block;">
                <div style="font-size:11px;font-weight:900;color:#00C8FF;letter-spacing:0.4px;">📡 23 공공데이터 카탈로그</div>
                <div style="font-size:9.5px;color:#7C8AA8;margin-top:3px;">전체 23 소스 mode/age/origin/derived 라이브 JSON</div>
              </a>
              <a href="/policy/" style="text-decoration:none;background:#070C16;border:1px solid #1A2438;border-radius:8px;padding:10px 12px;display:block;">
                <div style="font-size:11px;font-weight:900;color:#00E09A;letter-spacing:0.4px;">⚖ 정책 의사결정</div>
                <div style="font-size:9.5px;color:#7C8AA8;margin-top:3px;">위험 hotspot Top10 + 정책 제안 + risk-breakdown drill-down</div>
              </a>
              <a href="/fusion/risk-breakdown/1007" style="text-decoration:none;background:#070C16;border:1px solid #1A2438;border-radius:8px;padding:10px 12px;display:block;" target="_blank">
                <div style="font-size:11px;font-weight:900;color:#A095FF;letter-spacing:0.4px;">⚙ Fusion 기여도 분해</div>
                <div style="font-size:9.5px;color:#7C8AA8;margin-top:3px;">한양대 1007 · 23 소스 contribution = value × weight 분해 JSON</div>
              </a>
              <a href="/bev3d/" style="text-decoration:none;background:#070C16;border:1px solid #1A2438;border-radius:8px;padding:10px 12px;display:block;">
                <div style="font-size:11px;font-weight:900;color:#7C3AED;letter-spacing:0.4px;">🎮 3D BEV (Three.js)</div>
                <div style="font-size:9.5px;color:#7C8AA8;margin-top:3px;">실 카메라 + ML Kit + 4방향 횡단보도 + Bloom</div>
              </a>
              <a href="/fleet/" style="text-decoration:none;background:#070C16;border:1px solid #1A2438;border-radius:8px;padding:10px 12px;display:block;">
                <div style="font-size:11px;font-weight:900;color:#FF8866;letter-spacing:0.4px;">📊 fleet 라이브</div>
                <div style="font-size:9.5px;color:#7C8AA8;margin-top:3px;">23 src grid + 위험점수 기여도 차트 (전체)</div>
              </a>
            </div>
          </div>
        </div>
              ...
              try {
                ...
                if (ver && ver.components) {
                  ...
                  const hlHtml = Object.entries(ver.components).map(function (e) {
                    ...
                    return '<div style="background:#070C16;border:1px solid #1A2438;border-radius:7px;padding:9px 11px;text-align:center;"><div style="font-size:8.5px;letter-spacing:1.6px;color:#7C8AA8;font-weight:800;margin-bottom:3px;">' + esc(k.replace(/_/g,' ')) + '</div><div style="font-size:13px;font-weight:900;color:' + (cls==='danger'?'#FF4040':'#00E09A') + ';font-family:monospace;">' + sign + ' ' + detail + '</div></div>';
                  }).join('');
                  document.getElementById('avHl').innerHTML = hlHtml;
                }
                // hotspot 마커는 지도 위에 유지 (정책 페이지로 drill-down)
