...
def build():
    ...
    r = p.add_run(
        "한국도로공사·한국교통안전공단·도로교통공단·국토교통부·기상청·환경부·소방청·"
        "보건복지부·경찰청·행정안전부·서울시 등 11개 기관 공공데이터 25종 (주관기관 융합 7종 포함)"
    )
    kfont(r, size=10)
    ...
    r = p.add_run(
        "스마트폰 카메라 1대로 25종 공공데이터를 실시간 융합하여 평균 3.38초 사전 위험을 알리는 한국 도로 특화 AI 블랙박스. "
        "가명결합·DSZ 준수, 자체 Transformer(AUC 0.9403) 적용으로 회피 성공률 25% → 84.5% 향상. "
        "전국 도입 시 연 사망 421명·사회비용 5조 6천억 원 절감."
    )
    kfont(r, size=10)
